...
v0.9 expands AI/ML and creative-workflow coverage and sharpens the `app_cache` source_label granularity without changing the L1-L4 risk model, the redaction rules, or any Stage 4–5 confirmation bar. Milestone M1 (rules-only, zero `scripts/scan_projects.py` changes) lands first:

- **Tier E gains five rows** (all `pkg_cache`, dir-probe in Stage 2): Conda / Mamba envs (per non-`base` env, L2 `trash`, `mode_hit_tags=["deep"]`; scope stops at `~/...` to avoid system `/opt/miniconda3/envs`), Playwright (macOS Caches + XDG fallback + driver-binaries, L1 `delete`), Puppeteer (L1 `delete`), OpenAI Whisper (L2 `trash`; Faster-Whisper is explicitly redirected to the HuggingFace row), Weights & Biases global cache (L2 `trash`). `source_label` table grows by four labels (`"Conda environment"` / `"Playwright browsers"` / `"Puppeteer browsers"` / `"Whisper model cache"` / `"Weights & Biases cache"`).
...
- **Creative-app source_label refinement.** `category-rules.md` §4 gains a refinement table mapping Adobe Media Cache / Peak Files / Final Cut Pro / Logic Pro / GarageBand sample paths to specific `source_label` values. Risk grading is unchanged (L2 `trash` via the Tier C-adjacent override); only the UI label becomes specific so the report names the actual workflow rather than `"System caches"`.
...
Cross-run history (`history.json`) stays format-compatible: new source_labels are just new tags. No `scripts/scan_projects.py` changes in M1. The M2 (Xcode DeviceSupport active-OS detection, `scan_projects.py` learning a `nested_cache` subtype for `.dvc/cache`, and version_pins emission for pyenv / nvm) and M3 (`ollama:<name>:<tag>` semantic dispatcher with blob refcounting) milestones land in follow-up commits and are rolled up into a single v0.9 ship PR alongside the README translations.
