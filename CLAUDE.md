...
- **Install path switched from manual to skillx.** The Install section's `git clone` + `mkdir -p ~/.claude/skills` + `ln -s` triplet is replaced by the skillx CLI bootstrap (`curl -fsSL https://skillx.run/install.sh | sh`) followed by `skillx install <github-url>`. skillx detects the harness's skills directory itself (Claude Code, Cursor, etc. — see `skillx agents`), so the README no longer hard-codes `~/.claude/skills/`. Followups (`skillx update mac-space-cleanup` / `skillx uninstall mac-space-cleanup`) get a one-line mention so contributors don't reach for `rm -rf` on the install dir. Manual git clone is no longer documented as an end-user path; the Development section still implies it for contributors.
...
v0.14 does not touch `README.md` or any of the 7 translations, so `§"Translated READMEs"`'s mirroring obligation — which is triggered only by substantive edits to `README.md` — never enters play. The changes live entirely in `SKILL.md` / `assets/report.css` / `scripts/validate_report.py` / `assets/i18n/strings.json` / `tests/`, none of which is surfaced in any README. The next substantive README edit picks up any naturally accumulated drift.
