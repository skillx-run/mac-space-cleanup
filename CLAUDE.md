...
Milestone M2 extends `scripts/scan_projects.py` and promotes two long-standing "documented but unimplemented" lookups into real code:

- **`scan_projects.py` learns a `nested_cache` artifact kind** for the "keep parent, drop cache-child" pattern — currently just `.dvc/cache`. The sibling `.dvc/config` is a new nested-path entry in `PROJECT_MARKERS` (marker-gated at Stage 4 per `category-rules.md` §10d), and `_detect_markers`'s existing `os.path.join` + `isfile` logic handles flat and nested paths uniformly. Surfacing the pair separately lets the agent reclaim the content-addressed cache without wrecking the DVC repo's user state. Default: L2 `trash` (rebuild requires network pull from the configured remote).
- **`scan_projects.py` emits `version_pins`** per project by reading `.python-version` and `.nvmrc` in the same single-pass walk. `_parse_version_pin_file` tolerates multi-version pyenv chains (`3.11.4 3.10.8`), comment lines, CRLF, empty files, and unreadable files. SKILL.md Stage 3 now takes the union of `version_pins.python` / `version_pins.node` across all scanned projects and appends it to the pyenv / nvm exclusion set — replacing the v0.4 "`cd` into each project and run `pyenv local`" choreography that was never wired up.
- **iOS DeviceSupport active-OS downgrade** (references-only; no script change). Stage 2 now collects active iOS versions from two JSON sources: `xcrun devicectl list devices --json-output -` (Xcode 15+; physical paired devices) and `xcrun simctl list devices available --json` (all Xcode versions; installed simulator runtimes). The union seeds `environment_profile.active_ios_versions`, and Stage 4 downgrades any `iOS DeviceSupport/<OS-version>` entry whose prefix matches from L2 `trash` to L3 `defer`. Empty union is a first-class "no hint" state — every entry simply stays at the L2 default. `watchOS` / `tvOS` DeviceSupport have no equivalent source and remain L2.

Cross-run history stays format-compatible — `version_pins` is a new sibling field, and consumers that do `"X" in markers_found` membership checks (the existing pattern) simply never see it. `projects.json`'s `kind` enum grows by one value (`nested_cache`); the same back-compat contract as v0.7's `coverage` addition.
...
Milestone M3 closes the v0.8 per-model gap for Ollama by introducing the first non-vendor-CLI semantic dispatcher:
...
- **SKILL.md Stage 3 branches on mode** for Ollama: quick mode keeps the v0.8 single-aggregate `~/.ollama/models/` L3 defer entry; deep mode walks `manifests/<registry>/<namespace>/<name>/<tag>` and emits one `ollama:<name>:<tag>` candidate per leaf with `source_label="Ollama model"`, `risk_level=L3`, `action=defer`, `mode_hit_tags=["deep"]`. Per-model entries REPLACE the aggregate in deep mode — both would double-count bytes. Fall back to the aggregate if the manifest walk fails for any reason (permission error, unexpected layout).
...
- **`tests/test_safe_delete_ollama.py` is new** with 8 cases: shared-blob preservation on single delete, cascading cleanup across sequential deletes, dry-run accounting with no disk writes, corrupt-manifest redaction-safe failure (the full manifest path is deliberately kept out of `actions.jsonl`), three malformed path forms, third-party `hf.co/...` registry mapping, default-registry user-namespace mapping, and an explicit blocklist-bypass smoke. All 122 tests pass (+8).

LM Studio stays on the single aggregate in v0.9 — its storage layout is less uniform across versions and a dispatcher has not yet been written. The `"Ollama model"` source_label (singular) is new for per-model entries; `"Ollama model cache"` (plural / aggregate) remains for quick-mode and the fallback path.
