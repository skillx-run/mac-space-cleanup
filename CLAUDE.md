...
Cross-run history stays format-compatible — `version_pins` is a new field but consumers that hardcode `markers_found` iteration skip it silently. `projects.json`'s `kind` enum grows by one value (`nested_cache`); the same back-compat contract as v0.7's `coverage` addition.
