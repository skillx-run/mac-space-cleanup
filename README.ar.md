...
تريد معاينة أولاً؟ أضف `تجربة جافة` إلى رسالتك. يمرّ الـ skill بالمراحل السبع كلها، لكن `safe_delete.py` لا يكتب شيئاً إلى نظام الملفات (سوى `actions.jsonl` في workdir).

```bash
skillx run --skip-scan --auto https://github.com/skillx-run/mac-space-cleanup "نظّف جهاز Mac، لكن في وضع تجربة جافة، دون حذف أي شيء فعلياً."
```
...
طرق تحرير مساحة القرص لا تنقص —— تطبيقات GUI مخصّصة (CleanMyMac، OnyX، DaisyDisk)، أو prompt LLM مباشر («مرحباً Claude، نظِّف جهاز Mac»)، أو الذاكرة العضلية `rm -rf ~/Library/Caches`. يوجد هذا الـ skill لأنّ الطرق الثلاث تترك ثغرات تُهِمّ على Mac المطوِّر.
...
باختصار: **أدوات GUI آمنة لكنّها معتمة وتضخِّم الرقم. LLM مباشر مرن لكنّه سيُنفِّذ `rm -rf` على الشيء الخطأ بسعادة. يبقي هذا الـ skill على مرونة الـ LLM ويضيف guardrails** —— blocklist حتميّة في الشيفرة، طبقة redaction لا يستطيع النموذج تجاوزها، ومحاسبة صادقة تجعل الرقم الظاهر على بطاقة المشاركة هو الرقم الذي خرج فعلاً من القرص.
...
للمعاينة دون لمس نظام الملفات، أضف عبارة `تجربة جافة` إلى رسالتك:

> «نظّف جهاز Mac، لكن في وضع تجربة جافة، دون حذف أي شيء فعلياً.»
