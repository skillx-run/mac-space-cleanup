...
تريد معاينة أولاً؟ أضف `--dry-run` إلى رسالتك. يمرّ الـ skill بالمراحل السبع كلها، لكن `safe_delete.py` لا يكتب شيئاً إلى نظام الملفات (سوى `actions.jsonl` في workdir).
...
ثمّة طريقتان لتحرير مساحة القرص على Mac، ولكلتيهما قيد جوهري:

**أدوات GUI التقليدية** (CleanMyMac، OnyX، DaisyDisk) تعمل من **قوائم مسارات مُرمَّزة بثبات في الشيفرة**. يغطّي ذلك ما هو بديهي —— `~/Library/Caches`، السجلّات، المُثبِّتات القديمة —— لكنّه يفوِّت معظم ما يأكل القرص فعلاً على Mac المطوِّر: 40 GB من أوزان HuggingFace، 12 GB من نماذج Ollama، `node_modules` مبعثرة عبر مشاريع ميتة، إصدارات غير نشطة من `pyenv` / `nvm`، أزمنة تشغيل محاكيات iOS ميتة. الأداة لا تستطيع قراءة `.python-version` ولا `.nvmrc`، لا تُميِّز بين `build/` و`src/`، ولا بين مُنتَج تمّ تجميعه للتوّ تحتاجه وبين كاش محض. القواعد الثابتة تفوِّت معظم ما يمكن لـ LLM التعامل معه بمعرفة السياق.

**prompt LLM مباشر** («مرحباً Claude، نظّف جهاز Mac») لديه بالضبط الذكاء الذي تفتقده أداة GUI —— النموذج **يستطيع** قراءة ملفات المشروع، فهم الاصطلاحات، اتخاذ الأحكام. لكنّ المرونة نفسها تجعله **خطراً بلا guardrails**: هلوسة واحدة واثقة يشغِّل النموذج `rm -rf` على `.git` أو `.env` أو Keychains أو سجلّ تحريرات VSCode غير المحفوظة. لا قائمة رفض حتميّة، لا تصنيف مخاطر لكلّ عنصر، لا redaction قبل أن يرى النموذج مساراتك، لا محاسبة صادقة لما خرج فعلاً من القرص.

**يجمع هذا الـ skill بين حكم الـ LLM وطبقة أمان حتميّة.** يتخذ الـ agent القرارات الذكيّة التي لا تستطيعها أداة GUI —— موزِّع Ollama لكلّ نموذج مع احتساب مراجع blobs المشتركة، قراءة `.nvmrc` لتخطّي إصدار Node النشط، تخفيض بنود `DeviceSupport/` التي تطابق major.minor جهاز iOS مقترن حالياً. لكنّ كلّ كتابة تمرّ عبر `safe_delete.py` وقائمة الحجب فيه ترفض `.git` / `.ssh` / Keychains / `.env*` / حالة المحرِّر غير المحفوظة **مهما قال الـ agent**. تُصنَّف المخاطر L1–L4 مع تأكيد لكلّ عنصر من L2 فأعلى، تُخفَّى المسارات إلى `source_label` + `category` قبل الوصول إلى التقرير، ورقم الاسترداد يُقسَم بين ما خرج فعلاً من القرص وما لا يزال في سلّة المهملات.

مقارنة حسب الأبعاد:

| ما يهمّك | أداة GUI تقليدية | prompt LLM مباشر | هذا الـ skill |
| --- | --- | --- | --- |
| **أين تقع الكتابات** | محرّك ملكيّ مغلق المصدر | أيّ `rm -rf` يقرّره النموذج | نقطة اختناق واحدة `safe_delete.py` مع blocklist حتميّة (`.git`، `.ssh`، Keychains، `.env*`، Adobe `Auto-Save`، تحريرات VSCode غير المحفوظة، …) تُطبَّق **قبل** استدعاء نظام الملفات —— ترفض حتى لو طلبت ذلك |
| **الوعي بالمخاطر** | عادةً سلّة واحدة «Safe to remove» | لا شيء —— النماذج تهلوس | تصنيف L1–L4 لكلّ عنصر. وضع Quick ينفِّذ L1 تلقائياً فقط. وضع Deep يسأل لكلّ عنصر في L2/L3. L4 لا يُنفَّذ تلقائياً أبداً |
| **صدق رقم الاسترداد** | «حرَّرنا 40 GB» كثيراً ما يحسب البايتات التي لا تزال في سلّة المهملات | ما يدَّعيه النموذج | مقسَّم إلى `freed_now` (خرجت فعلاً من القرص) / `pending_in_trash` / `archived_source`. عنوان نصّ المشاركة يستخدم `freed_now` |
| **الخصوصية عند الخروج من الجهاز** | محلّي لكنّه غير شفّاف | مسارات + أسماء ملفات كاملة تُرسَل إلى المزوِّد | يصل إلى التقرير فقط `source_label` + `category`. sub-agent للمراجعة (redaction reviewer) و validator بعد العرض يلتقطان التسرّب قبل أن ترى HTML |
| **الوعي بـ Mac المطوِّر** | مسح عامّ للمجلّدات | محادثة فقط، بلا مسح | اكتشاف مشاريع تبدأ بـ `.git`؛ موزِّع Ollama لكلّ نموذج (`ollama:<name>:<tag>`) مع احتساب مراجع blobs؛ تخفيض إصدار iOS النشط في `DeviceSupport/`؛ استبعاد version-pin (`.python-version` / `.nvmrc`) لِـ nvm/pyenv |
| **التدقيق وإعادة التشغيل** | عادةً لا يوجد | سجلّ المحادثة فقط | `actions.jsonl` بالإلحاق فقط لكلّ تشغيلة. متماثل —— المسارات المفقودة تتحوّل إلى `skip/success`، إعادة التشغيل على نفس workdir آمنة |
| **Dry-run** | نادر أو مدفوع | طلب من النموذج «لا تفعل حقاً» | درجة أولى —— كلّ المراحل تمرّ، `safe_delete.py` لا يكتب شيئاً، التقرير يحمل لافتة `DRY-RUN` |
| **الانفتاح** | منتج تجاري مغلق المصدر | بدون guardrails على مستوى المصدر | Apache-2.0، صفر اعتماديات pip، أوامر macOS ومكتبة Python القياسية فقط |
