...
**أدوات GUI التقليدية** (CleanMyMac، OnyX، DaisyDisk) تعمل بقواعد ثابتة —— لا تستطيع الحكم على حالتك المحدَّدة. لا تعلم هل `~/Datasets/` لديك بيانات بحث لا يمكن استعادتها أم كاش مؤقّت، هل `node_modules` معيَّن يعود لمشروع نشط أم ميت، هل ذلك النموذج Ollama بحجم 12 GB ما زلت تستخدمه أم نسيته منذ زمن.

**prompt LLM مباشر** («Claude، نظّف جهاز Mac») لديه الحكم الذي تفتقده أداة GUI —— لكن بلا guardrails. هلوسة خاطئة واحدة يشغِّل `rm -rf` على `.git` أو `.env` أو Keychains.

**يجمع هذا الـ skill** بين الاثنين: يقرأ الـ agent نظامك في السياق ويتخذ القرارات، لكنّ كلّ كتابة تمرّ عبر `safe_delete.py` وقائمة الحجب فيه ترفض `.git` / `.ssh` / Keychains / `.env*` مهما قال الـ agent.
