...
| **الخصوصية عند الخروج من الجهاز** | محلّي لكنّه غير شفّاف | مسارات + أسماء ملفات كاملة تُرسَل إلى المزوِّد | يصل إلى التقرير فقط `source_label` + `category`. sub-agent `redaction reviewer` و validator بعد العرض يلتقطان التسرّب قبل أن ترى HTML |
