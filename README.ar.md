...
أدوات التنظيف التقليدية (CleanMyMac، OnyX) تعمل بقواعد ثابتة —— **لا تستطيع الحكم على حالتك**، لا تُميِّز `node_modules` نشطاً عن متروك ولا بيانات لا تُعوَّض عن كاش عديم القيمة. agent بلا حماية («Claude، نظّف جهاز Mac») لديه ذلك الحكم، لكن **بلا guardrails** —— قرار خاطئ واحد و `rm -rf` يضرب `.git` أو `.env` أو Keychains.

يجمع هذا الـ skill بين الاثنين: حكم الـ agent في المقدمة، وقائمة الحجب الحتميّة في `safe_delete.py` في الخلف. **الحكم للـ agent، الخطّ الأخير للـ blocklist.**
