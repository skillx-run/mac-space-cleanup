...
先にプレビューしてみたい？ トリガー文に `--dry-run` を追加してください。skill は 7 つのステージすべてを実行しますが、`safe_delete.py` はファイルシステムに何も書き込みません（workdir の `actions.jsonl` のみ）。
...
Mac の空き容量を確保する既存の方法は 2 つ、どちらも根本的な限界があります：

**従来型の GUI クリーナー**（CleanMyMac、OnyX、DaisyDisk）は**ハードコードされたパス一覧**で動きます。明らかな対象 —— `~/Library/Caches`、ログ、古いインストーラ —— には効きますが、開発者の Mac で本当に容量を食っているものの大半を見逃します：40 GB の HuggingFace ウェイト、12 GB の Ollama モデル、廃プロジェクトに散らばった `node_modules`、非アクティブな `pyenv` / `nvm` バージョン、死んだ iOS シミュレータ ランタイム。クリーナーはあなたの `.python-version` や `.nvmrc` を読めず、`build/` と `src/` を区別できず、コンパイルしたての必要な成果物と純粋なキャッシュの違いがわかりません。固定ルールは、LLM がコンテキストで判断できる場面の大半を取り逃がします。

**素の LLM プロンプト**（「Claude、Mac を掃除して」）は、GUI ツールに欠けている知能をまさに持っています —— モデルはプロジェクトファイルを**読める**、慣習を理解する、判断を下す。しかし同じ柔軟性が、**ガードレールがないと危険**にします：一度の自信に満ちたハルシネーションで、モデルがあなたの `.git` / `.env` / Keychains / VSCode 未保存編集履歴に `rm -rf` を走らせます。決定的な拒否リストなし、項目ごとのリスクグレーディングなし、モデルがパスを見る前の redaction なし、実際にディスクから離れた分の誠実な会計もなし。

**この skill は LLM の判断力と決定的な安全層を組み合わせます。** Agent は GUI クリーナーができないスマートな判断をします —— shared-blob 参照カウント付きのモデル単位 Ollama ディスパッチ、`.nvmrc` を読んでアクティブ Node バージョンを除外、現在ペアリング中の iOS デバイスに major.minor が一致する `DeviceSupport/` エントリの降格。しかし、どの書き込みも `safe_delete.py` を通り、その blocklist は **agent が何を言おうと** `.git` / `.ssh` / Keychains / `.env*` / 未保存エディタ状態を拒否します。リスクは L1–L4 で分類（L2 以上は項目ごとに確認）、パスはレポートに入る前に `source_label` + `category` にマスキング、回収の数字は「本当にディスクから離れた分」と「Trash にまだあるもの」に分割。
