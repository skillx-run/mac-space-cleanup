...
**従来型の GUI クリーナー**（CleanMyMac、OnyX、DaisyDisk）は固定ルールで動きます —— あなたの具体的な状況を判断できません。あなたの `~/Datasets/` が再現不可能な研究データなのか一時キャッシュなのか、ある `node_modules` がアクティブなプロジェクトか廃プロジェクトか、12 GB の Ollama モデルがまだ使われているのか忘れられているのか、知りません。

**素の LLM プロンプト**（「Claude、Mac を掃除して」）は GUI ツールに欠けている判断力を持ちますが、ガードレールがありません。一度の間違ったハルシネーションで `.git` / `.env` / Keychains に `rm -rf` が走ります。

**この skill** は両者を組み合わせます：agent はあなたのシステムをコンテキストで読み取り判断を下しますが、すべての書き込みは `safe_delete.py` を通り、その blocklist は agent が何を言おうと `.git` / `.ssh` / Keychains / `.env*` を拒否します。
