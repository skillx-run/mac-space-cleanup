...
従来のクリーナー（CleanMyMac、OnyX）は固定ルールで動きます —— **あなたの状況を判断できません**。ある `node_modules` がまだアクティブか廃棄されたか、かけがえのないデータと不要なキャッシュの違いがわかりません。素の agent（「Claude、Mac を掃除して」）にはその判断力がありますが、**ガードレールがありません** —— 一度の間違った判断で `.git` / `.env` / Keychains に `rm -rf` が走ります。

この skill は両者を組み合わせます：前面に agent の判断力、背後に `safe_delete.py` の決定的 blocklist。**判断は agent に、兜底は blocklist に。**
