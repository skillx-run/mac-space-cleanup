...
- 开发者缓存：Xcode DerivedData、Docker build cache、Go build cache、Gradle cache、ccache、sccache。
- 包管理器缓存：Homebrew、npm、pnpm、yarn、pip、uv、Cargo、CocoaPods、RubyGems、Bundler、Composer、Poetry、Dart pub。
...
- **项目构建产物**（仅 deep 模式；通过 `scripts/scan_projects.py` 扫描任何带 `.git` 根的目录）：
  ...
  - L2 移入 Trash：`.venv`、`venv`、`env`（Python 虚拟环境——wheel 版本钉住后可能无法完全复现，所以留个回收窗口）；`coverage`（测试覆盖率报告，按 `package.json` 或 Python marker 判定）。
...
- **Deep 模式还会扫描 `~` 下 ≥ 2 GiB 且不匹配任何其他规则的目录**（L3 defer，`source_label="Unclassified large directory"`），把真正的孤儿大目录暴露给用户手动判断。
