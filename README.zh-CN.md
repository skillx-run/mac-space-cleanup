...
想先预演一下？在触发语里加上 `预演`。skill 会走完七个阶段，但 `safe_delete.py` 不会写入文件系统（只写运行目录的 `actions.jsonl`）。

```bash
skillx run --skip-scan --auto https://github.com/skillx-run/mac-space-cleanup "深度清理一下我的 Mac，用预演模式先看一眼，不要真的删任何文件。"
```
...
释放 Mac 磁盘空间的方案早已不缺——专门的 GUI 清理工具（CleanMyMac、OnyX、DaisyDisk）、直接给 LLM 一句话（"嘿 Claude，清理一下我的 Mac"），或者凭肌肉记忆 `rm -rf ~/Library/Caches`。这个 skill 存在的理由是：三种方案在开发者 Mac 上都有各自的盲区。
...
一句话总结：**GUI 清理工具安全但不透明、还虚报数字。裸 LLM 灵活但会随手 `rm -rf` 错东西。这个 skill 保留了 LLM 的灵活，加上 guardrail** —— 代码里的确定性 blocklist、模型绕不开的 redaction 层、以及诚实的回收口径，让分享卡上的数字就是真正从磁盘腾出来的数字。
...
想预演一遍但不真的改文件，在你的触发语里加上 `预演` 关键字：

> "深度清理一下我的 Mac，用预演模式先看一眼，不要真的删任何文件。"
