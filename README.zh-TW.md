...
- 開發者快取：Xcode DerivedData、Docker build cache、Go build cache、Gradle cache、ccache、sccache。
- 套件管理器快取：Homebrew、npm、pnpm、yarn、pip、uv、Cargo、CocoaPods、RubyGems、Bundler、Composer、Poetry、Dart pub。
...
- **專案建置產物**（僅 deep 模式；透過 `scripts/scan_projects.py` 掃描任何帶 `.git` 根的目錄）：
  ...
  - L2 移入 Trash：`.venv`、`venv`、`env`（Python 虛擬環境——wheel 版本固定後可能無法完全重現，所以留個回收期）；`coverage`（測試覆蓋率報告，以 `package.json` 或 Python marker 判定）。
...
- **Deep 模式另會掃描 `~` 下 ≥ 2 GiB 且不符合其他規則的目錄**（L3 defer，`source_label="Unclassified large directory"`），讓真正孤兒的大目錄可被使用者手動判定。
