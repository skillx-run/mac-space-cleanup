...
釋放 Mac 磁碟空間的方案早已不缺——專門的 GUI 清理工具（CleanMyMac、OnyX、DaisyDisk）、直接給 LLM 一句話（「嘿 Claude，清理一下我的 Mac」），或者憑肌肉記憶 `rm -rf ~/Library/Caches`。這個 skill 存在的理由是：三種方案在開發者 Mac 上都有各自的盲區。
...
一句話總結：**GUI 清理工具安全但不透明、還虛報數字。裸 LLM 靈活但會隨手 `rm -rf` 錯東西。這個 skill 保留了 LLM 的靈活，加上 guardrail** —— 程式碼裡的確定性 blocklist、模型繞不開的 redaction 層、以及誠實的回收口徑，讓分享卡上的數字就是真正從磁碟騰出來的數字。
