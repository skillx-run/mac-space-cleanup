...
一個清理 Mac 磁碟空間的 **skill** —— 謹慎、誠實、多階段。

> 本 skill 透過七階段工作流程（模式選擇 → 環境探測 → 掃描 → 分級 → 二次確認 → 報告 → 開啟）驅動 agent 完成清理，具備 **L1–L4 風險分級**、**誠實的回收量統計**（拆分為 `freed_now` / `pending_in_trash` / `archived` 三項）和**多重安全兜底**（程式碼內建的確定性封鎖清單、一個負責隱私脫敏的 reviewer 子 agent，以及渲染後驗證器）。零 pip 相依——純 macOS 指令加 Python 標準函式庫。
...
想先預演一下？在觸發語裡加上 `--dry-run`。skill 會走完七個階段，但 `safe_delete.py` 不會寫入檔案系統（只寫執行目錄的 `actions.jsonl`）。
...
目前 Mac 釋放磁碟空間有兩種主流方案，兩種都有一個根本的局限：

**傳統 GUI 清理工具**（CleanMyMac、OnyX、DaisyDisk）基於**寫死的路徑清單**工作。處理顯而易見的目標還行 —— `~/Library/Caches`、日誌、舊安裝包 —— 但它看不到開發者 Mac 上真正吃磁碟的大部分東西：40 GB 的 HuggingFace 權重、12 GB 的 Ollama 模型、散在廢棄專案裡的 `node_modules`、非活躍的 `pyenv` / `nvm` 版本、已死的 iOS 模擬器 runtime。它讀不懂你的 `.python-version` / `.nvmrc`，分不清 `build/` 和 `src/`，也判斷不出一份剛編譯好、你其實需要的產物和一份純快取的區別。固定規則會錯過 LLM 能結合上下文判斷的絕大多數場景。

**裸 LLM prompt**（「嘿 Claude，清理一下我的 Mac」）恰好擁有 GUI 工具缺少的智能 —— 它**可以**讀專案檔案、理解慣例、做判斷。但同樣的靈活性讓它**沒有 guardrail 時非常危險**：一次自信的幻覺就可能對你的 `.git` / `.env` / Keychains / VSCode 未儲存編輯歷史 `rm -rf`。沒有確定性拒絕清單、沒有逐項風險分級、沒有在模型看到路徑前的 redaction、沒有對「真正離開磁碟多少」的誠實計量。

**這個 skill 把 LLM 的判斷力和確定性安全層組合在一起。** Agent 做 GUI 清理工具做不到的判斷 —— 按模型分發的 Ollama 分發器（帶 shared-blob 參照計數）、讀 `.nvmrc` 跳過活躍 Node 版本、把 major.minor 匹配現役 iOS 裝置的 `DeviceSupport/` 條目降級。但每一次寫入都要過 `safe_delete.py` 這一關，它的 blocklist **無論 agent 怎麼說**都會拒絕 `.git` / `.ssh` / Keychains / `.env*` / 未儲存的編輯器狀態。風險按 L1–L4 分級（L2+ 逐項確認），路徑在進報告之前脫敏為 `source_label` + `category`，回收數字拆成「真正離開磁碟」和「還在 Trash 裡」兩項。

逐維度對比：

| 你在意的點 | 傳統 GUI 清理工具 | 裸 LLM prompt | 本 skill |
| --- | --- | --- | --- |
| **檔案系統寫入入口** | 閉源專有引擎 | 模型隨意發起的 `rm -rf` | 單一的 `safe_delete.py` 收口點，**在**呼叫檔案系統之前執行確定性 blocklist（`.git`、`.ssh`、Keychains、`.env*`、Adobe `Auto-Save`、VSCode 未儲存編輯……）——即使你要求也拒絕刪除 |
| **風險意識** | 通常只有「Safe to remove」一個桶 | 無——模型會幻覺 | 按條目做 L1–L4 分級。Quick 模式只自動執行 L1。Deep 模式對 L2/L3 逐項確認。L4 絕不自動執行 |
| **回收數字誠實** | 「釋放了 40 GB」常把還在 Trash 裡的位元組也計入 | 模型說啥就是啥 | 拆為 `freed_now`（真正離開磁碟）/ `pending_in_trash` / `archived_source`。分享卡頭條用的是 `freed_now` |
| **離開本機的隱私** | 本地但不透明 | 完整路徑 + 檔名發往服務端 | 僅 `source_label` + `category` 進入報告。redaction reviewer 子 agent + 渲染後 validator 在你看到 HTML 之前攔下洩漏 |
| **開發者 Mac 認知** | 泛泛的目錄掃描 | 純聊天，不掃碟 | 以 `.git` 為根的專案發現；按模型分發的 Ollama 分發器（`ollama:<name>:<tag>`），帶 blob 參照計數；針對 `DeviceSupport/` 的 active-iOS-version 降級；針對 nvm/pyenv 的版本 pin（`.python-version` / `.nvmrc`）排除 |
| **稽核與重跑** | 通常沒有 | 只有聊天紀錄 | 每次執行一個 append-only 的 `actions.jsonl`。冪等——已消失的路徑變成 `skip/success`，對同一 workdir 重跑安全 |
| **Dry-run** | 很少見或需付費 | 請求模型「別真刪」 | 一等公民——每階段照跑，`safe_delete.py` 不寫入任何東西，報告帶 `DRY-RUN` 橫幅 |
| **開放性** | 閉源商業產品 | 原始碼層面無 guardrail | Apache-2.0，零 pip 相依，純 macOS 指令 + Python stdlib |
...
想預演一遍但不真的改檔案，在你的觸發語裡加上 `--dry-run`：
...
- AI/ML 模型快取:HuggingFace(`hub/` L2 trash、`datasets/` L3 defer)、PyTorch hub、Ollama(L3 defer;deep 模式下以 `ollama:<name>:<tag>` 按模型分派,透過 blob 引用計數讓不同 tag 共享的 layer 在刪除某一個 tag 時仍然保留)、LM Studio、OpenAI Whisper、Weights & Biases 全域快取。Conda / Mamba / Miniforge 的非 `base` env,涵蓋七種常見的 macOS 安裝配置。
- 前端工具鏈:Playwright 瀏覽器 + driver、Puppeteer 綁定的瀏覽器。
- iOS/watchOS/tvOS 模擬器執行環境(透過 `xcrun simctl delete`,**絕不使用 `rm -rf`**)。iOS `DeviceSupport/<OS>` 條目中 major.minor 與目前已配對實體裝置或可用模擬器 runtime 匹配的,會被自動降級為 L3 defer。
- `~/Library/Caches/*` 下的應用程式快取、saved application state、以及 Trash 本身。創作類應用程式快取(Adobe Media Cache / Peak Files、Final Cut Pro、Logic Pro)使用具體標籤而非通用的 `"System caches"`。
- 記錄檔、當機報告。
...
- **專案建置產物**（僅 deep 模式；透過 `scripts/scan_projects.py` 掃描任何帶 `.git` 根的目錄）：
  ...
  - L2 移入 Trash：`.venv`、`venv`、`env`（Python 虛擬環境——wheel 版本固定後可能無法完全重現，所以留個回收期）；`coverage`（測試覆蓋率報告，以 `package.json` 或 Python marker 判定）；`.dvc/cache`（DVC 的內容定址快取，以同層 `.dvc/config` marker 判定——父目錄 `.dvc/` 含使用者狀態，保留不動）。
  - 系統 / 套件管理器目錄（`~/Library`、`~/.cache`、`~/.npm`、`~/.cargo`、`~/.cocoapods`、`~/.gradle`、`~/.m2`、`~/.gem`、`~/.bundle`、`~/.composer`、`~/.pub-cache`、`~/.local`、`~/.rustup`、`~/.pnpm-store`、`~/.Trash`）會在專案發現階段被剪枝。
- **Deep 模式另會掃描 `~` 下 ≥ 2 GiB 且不符合其他規則的目錄**（L3 defer，`source_label="Unclassified large directory"`），讓真正孤兒的大目錄可被使用者手動判定。在最終定稿前 agent 會做一小段唯讀調查（每個候選最多 6 次命令）以細化 category 和 source_label；無論調查結果如何，L3 defer 的風險檔位都被鎖定。
