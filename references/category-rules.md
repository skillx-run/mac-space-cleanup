...
- **iOS DeviceSupport active-OS downgrade** (v0.9+): normalize the entry's subdir name to `major.minor` (e.g. `17.4.1 (21E236)` → `17.4`) and compare as equal strings against `environment_profile.active_ios_versions` — which Stage 2 also populates in `major.minor` form from `xcrun devicectl list` + `xcrun simctl list devices available`. On match, override to **L3 `defer`**: the user is almost certainly debugging against that OS and a re-pull-triggered 10+ minute stall is the kind of surprise Stage 5 defer is designed to prevent. `active_ios_versions` being empty (no Xcode 15+ `devicectl`, no paired devices, no available simulators) simply leaves every entry at the L2 default; the downgrade is additive, never a required step. `watchOS` / `tvOS` DeviceSupport have no equivalent active-OS source in v0.9 and always stay L2.
...
- `~/.ollama/models/**` (Ollama local LLM store; quick-mode aggregate — one item. Deep mode replaces this with per-model semantic entries under the `ollama:<name>:<tag>` scheme dispatched by `_handle_ollama_delete` in `scripts/safe_delete.py`. Registry resolution for the `<name>:<tag>` suffix mirrors the Ollama client: first segment with `.` → literal `<host>/<rest>`; any `/` but no `.` in the first segment → default registry + user namespace; plain name → default registry + `library` namespace. The dispatcher reads the target manifest, walks every other manifest in the tree, and deletes only blobs whose digests no sibling still references — so a shared layer between `llama3:8b` and `llama3:70b` survives the delete of one tag.)
...
- Ollama and LM Studio model stores — all variants set **L3** `defer` (multi-GB local LLMs the user pulled deliberately; stays L3 so every delete requires explicit per-item confirmation at Stage 5). The three entries split by dispatch strategy:
  - `~/.ollama/models/**` quick-mode aggregate — `mode_hit_tags=["quick"]`. Deep mode does NOT surface this item because per-model entries below replace it (both would double-count bytes). One exception: if the manifest walk fails (missing dir, permission error, unexpected layout), Stage 3 falls back to emitting the aggregate in deep mode as a last resort.
  - `ollama:<name>:<tag>` per-model semantic entries (v0.9+) — `mode_hit_tags=["deep"]`. Dispatched by `_handle_ollama_delete` in `safe_delete.py`, which reference-counts blob digests across every manifest so shared layers survive the delete of one sibling tag.
  - `~/.cache/lm-studio/models/**`, `~/.lmstudio/models/**` LM Studio aggregate — `mode_hit_tags=["quick","deep"]` in both modes (no per-model dispatcher yet; storage layout is less uniform across versions).
...
| `pkg_cache` | `"Homebrew cache"`, `"Homebrew Cellar cleanup"`, `"npm cache"`, `"pnpm store"`, `"Yarn Berry cache"`, `"Bun cache"`, `"Deno cache"`, `"pip cache"`, `"uv cache"`, `"Cargo cache"`, `"Swift PM cache"`, `"Carthage cache"`, `"Android SDK image"`, `"Node version manager"`, `"Python version manager"`, `"Rust toolchain"`, `"RubyGems cache"`, `"Bundler cache"`, `"Composer cache"`, `"Poetry cache"`, `"ccache"`, `"sccache"`, `"Dart pub cache"`, `"HuggingFace model cache"`, `"HuggingFace dataset cache"`, `"PyTorch hub cache"`, `"Ollama model cache"` (quick-mode aggregate), `"Ollama model"` (per-model, deep mode only), `"LM Studio model cache"`, `"Conda environment"`, `"Playwright browsers"`, `"Puppeteer browsers"`, `"Whisper model cache"`, `"Weights & Biases cache"` |
