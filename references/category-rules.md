...
- `~/.ollama/models/**`, `~/.cache/lm-studio/models/**`, `~/.lmstudio/models/**` — set **L3** `defer`, `mode_hit_tags=["quick","deep"]` for LM Studio / the Ollama aggregate, `mode_hit_tags=["deep"]` for per-model `ollama:<name>:<tag>` entries. Multi-GB local LLMs the user pulled deliberately — stays L3 so every delete requires explicit per-item confirmation at Stage 5. v0.9 enables per-model granularity for Ollama via the `_handle_ollama_delete` reference-counting dispatcher in `safe_delete.py`; LM Studio stays on a single aggregate because its storage layout is less uniform across versions and a dispatcher has not yet been written.
