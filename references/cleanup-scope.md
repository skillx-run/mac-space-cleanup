...
| `nvm` (`~/.nvm` present, dir probe — NVM is a shell function, no CLI on PATH) | `~/.nvm/versions/node/*` **except** (a) the version matching `~/.nvm/alias/default`, (b) any version used in the last 90 days (mtime on the version dir), and (c) any version surfaced in `projects.json`'s `version_pins.node` (v0.9+ — scan_projects reads `.nvmrc` across all projects). Surface as individual items, one per non-default/stale/unpinned version. | `pkg_cache` |
...
| `pyenv` (CLI probe) | `~/.pyenv/versions/*` **except** (a) the global default from `pyenv version --bare` (run once, no cwd needed) and (b) any per-project pin surfaced in `projects.json`'s `version_pins.python` (v0.9+ — `scripts/scan_projects.py` reads every scanned project's `.python-version` in one pass, so the agent takes the union of every project's list instead of `cd`-ing into each). Surface per-version. | `pkg_cache` |
...
| Ollama (`~/.ollama` present, dir probe — **no CLI probe** to avoid redundancy with the dir probe; surfacing `~/.ollama` after the user has uninstalled the `ollama` CLI is exactly the case we want to catch) | `~/.ollama/models/`. **Quick mode**: one aggregate item (the whole tree). **Deep mode (v0.9+)**: walk `manifests/<registry>/<namespace>/<name>/<tag>` and emit one `ollama:<name>:<tag>` semantic entry per leaf — dispatched by `_handle_ollama_delete` in `scripts/safe_delete.py`, which does reference counting over every manifest's blob digests so shared layers survive the delete of one sibling tag. Fall back to the aggregate if the manifest walk fails (missing dir, permission error, unexpected layout). | `pkg_cache` |
...
| Conda / Mamba / Miniforge (`~/miniconda3/envs`, `~/anaconda3/envs`, `~/opt/miniconda3/envs`, `~/opt/anaconda3/envs`, `~/miniforge3/envs`, `~/mambaforge/envs`, or `~/.mamba/envs` present — dir probe only; covers the five common macOS install layouts including miniforge / mambaforge on Apple Silicon. Scope deliberately limited to `~/...` to avoid system-managed `/opt/miniconda3/envs` which may lie under Hard-forbid blacklist and require privilege) | `<detected-root>/envs/*` — each env subdir under the detected install layout's `envs/` root is one item, **excluding `base`** (removing the `base` env is equivalent to uninstalling conda / mamba / miniforge). | `pkg_cache` |
| Playwright (`~/Library/Caches/ms-playwright`, `~/.cache/ms-playwright`, or `~/Library/Caches/ms-playwright-driver` present — dir probe only; Playwright writes to either the macOS default or the XDG fallback across versions, plus a separate driver-binaries cache) | `~/Library/Caches/ms-playwright/`, `~/.cache/ms-playwright/`, `~/Library/Caches/ms-playwright-driver/` (one aggregate item from whichever paths exist; `npx playwright install` fully regenerates all three). | `pkg_cache` |
| Puppeteer (`~/.cache/puppeteer` present, dir probe) | `~/.cache/puppeteer/` (bundled browser binaries; Puppeteer re-downloads on next `npm install` or programmatic launch). | `pkg_cache` |
| OpenAI Whisper (`~/.cache/whisper` or `~/.cache/openai-whisper` present, dir probe) | `~/.cache/whisper/`, `~/.cache/openai-whisper/` (one aggregate item from whichever path exists; per-model granularity deferred — users commonly accumulate 100–500 MB per model across multiple languages). **Faster-Whisper (SYSTRAN)** pulls from the HuggingFace hub and is covered by the HuggingFace row above. | `pkg_cache` |
| Weights & Biases (`~/.wandb` or `~/.cache/wandb` present, dir probe — W&B has shipped both layouts across versions: older SDKs write run state to `~/.wandb/`, newer SDKs use the XDG `~/.cache/wandb/` for artefacts) | `~/.wandb/`, `~/.cache/wandb/` (one aggregate item from whichever path exists; per-project `./wandb/` dirs are not swept — too scattered). | `pkg_cache` |
