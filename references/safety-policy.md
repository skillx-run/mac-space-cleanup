...
8. **Ollama per-model semantic paths stay L3 / `defer` through Stage 5, and history downgrades cannot flip them to auto-execute.** `ollama:<name>:<tag>` candidates emitted by Stage 3 in deep mode are **synthetic paths** — `_BLOCKED_PATTERNS` doesn't apply (no filesystem substring to match) and the idempotency existence check is bypassed (the manifest lookup happens inside the dispatcher). The `_handle_ollama_delete` reference-counting algorithm is the safety net that makes per-model granularity sound, not the user-facing L3 default. To preserve the "user sees every LLM delete" contract: the default `risk_level=L3` and `action=defer` are locked through Stage 5, **fully governed by the "Risk-level boundaries are never crossed" rule in §"History-driven UI adjustments"** — even a tag with `confirmed ≥ 3` and `declined == 0` only earns the per-item → batch-confirm collapse, never auto-execute. The `confirmed_items` gate at Stage 5 is still the authoritative explicit-consent step; no history of approvals bypasses it. A malformed `ollama:` path (missing tag, empty name) is surfaced as `status=failed` with a short, redaction-safe error — the full on-disk manifest path is deliberately omitted from `actions.jsonl` to avoid leaking user-specific directory fragments.
